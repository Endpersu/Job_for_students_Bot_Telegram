...
- Управлять своим профилем"""
    ...
    async def search_job(message: types.Message, state: FSMContext):
        ...
        if not profession:
            await message.answer("Вы не указали желаемую сферу деятельности.")
            await message.answer("Переходим в главное меню.", reply_markup=get_main_keyboard())
            return
        ...
        try:
            ...
            if vacancies.get("items"):
                for item in vacancies["items"][:10]:
                    ...
                    responsibility = clean_text(item['snippet']['responsibility'])
    ...
    async def show_profile(message: types.Message, state: FSMContext):
        ...
        📝 Фамилия: {user_data.get('second_name', 'Не указана')}
        🔢 Возраст: {user_data.get('age', 'Не указан')}
        💼 Сфера деятельности: {user_data.get('profession', 'Не указана')}
        """
...
def clean_text(text: str) -> str:
    """
    Удаляет HTML-подобные теги из строки (например, <highlighttext>Python</highlighttext>)
    """
    if text:
